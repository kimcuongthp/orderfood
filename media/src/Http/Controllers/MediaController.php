        ...
        switch ($request->input('view_in')) {
            ...
            case 'favorites':
                ...
                if (!empty($favorite_items)) {
                    ...
                    if ($file_ids || $folder_ids) {
                        if ($file_ids) {
                            $paramsFile['file_ids'] = $file_ids;
                        }

                        if ($folder_ids) {
                            $paramsFolder['folder_ids'] = $folder_ids;
                        }
    ...
    public function postGlobalActions(Request $request)
    {
        ...
        switch ($type) {
            ...
            case 'favorite':
                ...
                $selected_items = $request->input('selected', []);

                foreach ($selected_items as &$selected_item) {
                    unset($selected_item['full_url']);
                }

                if (!empty($meta->value)) {
                    $meta->value = array_merge($meta->value, $selected_items);
                } else {
                    $meta->value = $selected_items;
                }
